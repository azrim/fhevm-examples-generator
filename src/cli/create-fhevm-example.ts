...
async function scaffoldExample(options: CliOptions): Promise<ScaffoldResult> {
  ...
  try {
    ...
    console.log(`   📦 Cloning base template from: ${BASE_TEMPLATE_REPO}`);
    ...
    if (options.contractTemplate) {
      ...
      if (await fs.pathExists(contractTemplatePath)) {
        ...
        console.log(`   📝 Injecting contract: ${contractFileName}`);
        ...
        console.log(`   ✅ Committed contract`);
      }
    }
    ...
    if (options.testTemplate) {
      ...
      if (await fs.pathExists(testTemplatePath)) {
        ...
        console.log(`   🧪 Injecting test: ${testFileName}`);
    ...
    console.log(`   🔗 Adding placeholder remote: ${placeholderRemote}`);
